...
from sft.spa_sft_dataset import SFTDataset
...
class FSDPSFTTrainer(object):
    ...
    def __init__(self, config, device_mesh: DeviceMesh, ulysses_device_mesh: DeviceMesh):
        ...
        # local_model_path = '/projects/b1222/model_hub/hub/step_200'
        from verl.utils import hf_tokenizer
        self.tokenizer = hf_tokenizer(local_model_path, trust_remote_code=self.config.model.trust_remote_code)
        if self.config.data.chat_template is not None:
            raise ValueError('Apply Chat template from config is not supported yet.')

        # Setup special tokens for observation and prediction content
    ...
    def _build_dataloader(self):
        ...
        print("Validation files:", self.config.data.val_files)
    ...
    def _build_model_optimizer(self):
        ...
        with init_context():
            ...
            if hasattr(self.tokenizer, 'obs_content_token_id'):
                original_vocab_size = self.model.config.vocab_size
                new_vocab_size = len(self.tokenizer)
                if new_vocab_size != original_vocab_size:
                    self.model.resize_token_embeddings(new_vocab_size)
                    print(f"Resized token embeddings from {original_vocab_size} to {new_vocab_size}")
    ...
    def _create_observation_prediction_mask(self, input_ids, tokenizer):
        """Create a mask for loss computation.

        According to SPA paper (Section 2.2, Equation 1-2):
        - Loss IS computed on tokens inside <think>...</think> AND <answer>...</answer>
        - This includes: <observation>, <prediction>, and action content
        - Only the INPUT (User message with raw state) is masked (standard LLM behavior)

        The mask M_i = 1 for tokens in span(<think>,</think>) U span(<answer>,</answer>)

        Both SPA and termination study train on observation/prediction content.
        The only difference is termination study adds extra fields like <done_label>, <steps_left>, etc.

        This function returns a mask of ones (train on all response tokens).
        Input masking is handled separately by the loss_mask from the dataset.
        """
        batch_size, seq_len = input_ids.shape

        # Train on ALL response tokens (observation, prediction, termination fields, action)
        # This matches SPA paper behavior
        return torch.ones(batch_size, seq_len, dtype=torch.bool, device=input_ids.device)

    def _setup_special_tokens(self):
        """Setup special tokens if needed.

        Note: Per SPA paper, we train on ALL tokens inside <think> and <answer> tags,
        including <observation> and <prediction> content. No special masking tokens needed.

        This function is kept for potential future use (e.g., adding termination-specific tokens).
        """
        # Currently no special tokens needed - SPA trains on all response content
        pass
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        # Create observation/prediction mask to exclude these tags from loss
        obs_pred_mask = self._create_observation_prediction_mask(input_ids, self.tokenizer)
        obs_pred_mask = obs_pred_mask[:, :-1].reshape(-1).cuda()  # Remove last token and flatten
        ...
        with context:
            with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                if not use_sp:
                    ...
                    # Apply both loss_mask and obs_pred_mask
                    combined_mask = loss_mask.to(loss.device) & obs_pred_mask.to(loss.device)
                    ...
                    # Apply both loss_mask and obs_pred_mask
                    combined_mask = loss_mask & obs_pred_mask.to(full_loss.device)
                    loss = full_loss * combined_mask

                valid_token_this_rank = torch.sum(combined_mask)
