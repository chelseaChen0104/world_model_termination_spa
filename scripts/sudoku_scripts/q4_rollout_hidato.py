...
Loads:
  - π_θ from outputs/rl_b_h1_v8_anchor/final
  - f_φ from outputs/save_hidato5x4_f_phi_paper/final  (only if SAVE is in --methods)
  - Calibration JSON for τ_keep / τ_fb (optional)
  - Solver

Test puzzle source: t=0 (or any) state_struct from test_natural_policy.jsonl.
...
Usage (smoke run, 4 methods, 20 puzzles):
    python scripts/sudoku_scripts/q4_rollout.py \\
        ...
        --save_phi outputs/save_hidato5x4_f_phi_paper/final \\
        --calibration outputs/save_hidato5x4_f_phi_paper/calibration.json \\
        --eval data/hidato5x4/test_natural_policy.jsonl \\
        --n_puzzles 20 \\
...
from q4_episode_hidato import Episode  # noqa: E402
from q4_methods_hidato import (  # noqa: E402
    ...
    with eval_path.open() as f:
        for line in f:
            ...
            puzzles.append({
                ...
                "state_hash": h,
                # For Hidato, state_struct itself IS the state dict
                # (rows, cols, assignment, puzzle_id) — no "grid" subkey.
                "grid": state["state_struct"],
            })
            if len(puzzles) >= n:
                break
    return puzzles
