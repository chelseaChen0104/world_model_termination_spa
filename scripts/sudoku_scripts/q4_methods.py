"""Q4 Sudoku candidate-selection methods (paper §3.5 / Table 4).
...
from sudoku4_env import (  # noqa: E402
...
POLICY_SYSTEM_PROMPT = """You are solving a 4x4 Sudoku puzzle. Fill in empty cells (shown as .) with numbers 1-4 so that each row, column, and 2x2 box contains each number exactly once.
...
class PolicyClient:
    ...
    def eval_logprob(self, grid, action: ActionStruct) -> float:
        ...
        from sudoku4_env import action_text as _action_text
...
class SaveScorer:
    ...
    def _build_prompt(self, grid, action: ActionStruct):
        from sudoku4_env import action_text as _action_text
        state_text = render_state_b5(grid)
        msgs = [
    ...
    def score(self, grid, action: ActionStruct) -> Tuple[float, int]:
        ...
        next_state_text = render_state_b5(next_grid)
...
class PromptedScorer:
    ...
    def score(self, grid, action):
        from sudoku4_env import action_text as _at
        msgs = [
            {"role": "system", "content": PROMPTED_SYSTEM},
            {"role": "user", "content": PROMPTED_USER.format(
                state_text=render_state_b5(grid), action_text=_at(action))},
        ]
...
class OracleTerminationMethod(Method):
    ...
    def __init__(self, policy: PolicyClient, solver: Sudoku4Solver, K: int = 8):
        self.policy = policy
        self.solver = solver
        self.K = K
